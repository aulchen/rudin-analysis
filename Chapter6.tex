...
Let $f \in \RiemannIntable (\alpha)$ on $[a, b]$, and $P = x_0 < x_1 ... < x_n$ such that $U(P, f, \alpha) - L(P, f, \alpha) < \epsilon$. Let $t_1, t_2 ... t_n$ be a set of points such that $t_i \in [x_{i-1}, x_i]$. Then 

\[
U(P, f, \alpha) - \sum_{i = 1}^n f(t_i) (\alpha(x_i) - \alpha(x_{i-1})) < \epsilon
\]
and
\[
\sum_{i = 1}^n f(t_i) (\alpha(x_i)) - \alpha(x_{i-1}) - L(P, f, \alpha) < \epsilon
\]

\begin{proof}
Tautologically, $inf_{[x_{i-1} - x_i]} \leq f(t_i) \leq sup_{[x_{i-1} - x_i]}$.
